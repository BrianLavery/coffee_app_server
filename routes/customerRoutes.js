...
module.exports = (app) => {
	app.post('/api/user', async (req, res) => {
		const { name, email, coffee, feedback, ipAddress, ipCity, ipCountry } = req.body;
		...
		if (existingUser) {
			...
			const user = await new User({ name, email, coffee, feedback, ipAddress, ipCity, ipCountry }).save();
			res.send({
